    ...
    <div style={{ minHeight: "100vh", display: "flex", background: "#080808", fontFamily: "'DM Mono', monospace", position: "relative", overflow: "hidden" }}>
      ...
      <motion.div variants={stagger} initial="hidden" animate="show" style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", padding: "80px 72px", position: "relative", zIndex: 1 }}>
        <motion.div variants={reveal} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 72 }}>
          ...
          <span style={{ fontSize: 11, color: "#222", letterSpacing: "0.16em" }}>FIGHT PREDICTOR</span>
        </motion.div>
        <motion.p variants={reveal} style={{ fontSize: 10, color: "#e8ff5a", letterSpacing: "0.2em", marginBottom: 14 }}>ML-POWERED \u00b7 XGBOOST \u00b7 64.2% CV</motion.p>
        ...
        <motion.p variants={reveal} style={{ fontSize: 14, color: "#2a2a2a", lineHeight: 1.85, maxWidth: 380, marginBottom: 64 }}>
          19-feature XGBoost trained on 10,006 fights \u2014 striking, grappling, style matchup, SHAP explanations. Sign in to run predictions.
        </motion.p>
        <motion.div variants={reveal} style={{ display: "flex", gap: 40, paddingTop: 32, borderTop: "1px solid #111" }}>
          {[["64.2%","CV ACCURACY"],["10k","FIGHTS TRAINED"],["4,229","FIGHTERS"]].map(([v,l]) => (
            <div key={l}>
              ...
              <div style={{ fontSize: 9, color: "#1e1e1e", letterSpacing: "0.14em", marginTop: 4 }}>{l}</div>
            </div>
          ))}
        </motion.div>
      </motion.div>

      <div style={{ width: 1, background: "#111", margin: "80px 0" }} />
      ...
      <motion.div initial={{ opacity: 0, x: 28 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.75, delay: 0.25, ease }} style={{ width: 420, display: "flex", flexDirection: "column", justifyContent: "center", padding: "80px 56px", position: "relative", zIndex: 1 }}>
        <div style={{ display: "flex", background: "#0a0a0a", borderRadius: 8, padding: 3, marginBottom: 28, border: "1px solid #141414" }}>
          {["login","signup"].map(m => (
            <button key={m} onClick={() => { setMode(m); setError(""); setSuccess(""); }} style={{ flex: 1, padding: "9px 0", border: "none", borderRadius: 6, background: mode === m ? "#161616" : "transparent", color: mode === m ? "#f0f0ec" : "#222", fontSize: 10, fontFamily: "'DM Mono', monospace", letterSpacing: "0.12em", cursor: "pointer", transition: "all 0.15s" }}>{m === "login" ? "SIGN IN" : "SIGN UP"}</button>
          ))}
        </div>

        <div style={{ marginBottom: 10 }}>
          <div style={{ fontSize: 9, color: "#222", letterSpacing: "0.18em", marginBottom: 6 }}>EMAIL</div>
          <input type="email" value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === "Enter" && submit()} onFocus={() => setFocusedEmail(true)} onBlur={() => setFocusedEmail(false)}
            style={{ width: "100%", padding: "13px 14px", background: "#080808", border: `1px solid ${focusedEmail ? "#e8ff5a" : "#161616"}`, borderRadius: 8, color: "#f0f0ec", fontSize: 13, fontFamily: "'DM Mono', monospace", outline: "none", transition: "border-color 0.2s" }} />
        </div>
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 9, color: "#222", letterSpacing: "0.18em", marginBottom: 6 }}>PASSWORD</div>
          <input type="password" value={password} onChange={e => setPassword(e.target.value)} onKeyDown={e => e.key === "Enter" && submit()} onFocus={() => setFocusedPass(true)} onBlur={() => setFocusedPass(false)}
            style={{ width: "100%", padding: "13px 14px", background: "#080808", border: `1px solid ${focusedPass ? "#e8ff5a" : "#161616"}`, borderRadius: 8, color: "#f0f0ec", fontSize: 13, fontFamily: "'DM Mono', monospace", outline: "none", transition: "border-color 0.2s" }} />
        </div>
        ...
        <motion.button onClick={submit} disabled={loading} whileHover={{ opacity: loading ? 1 : 0.88 }} whileTap={{ scale: loading ? 1 : 0.97 }}
          style={{ width: "100%", padding: "14px 0", background: loading ? "#111" : "#e8ff5a", color: loading ? "#222" : "#000", border: "none", borderRadius: 8, fontSize: 11, fontWeight: 700, fontFamily: "'DM Mono', monospace", cursor: loading ? "not-allowed" : "pointer", letterSpacing: "0.14em", marginBottom: 28 }}
        >{loading ? "..." : mode === "login" ? "SIGN IN \u2192" : "CREATE ACCOUNT \u2192"}</motion.button>

        <div style={{ borderTop: "1px solid #111", paddingTop: 20 }}>
          <div style={{ fontSize: 10, color: "#181818", lineHeight: 1.9 }}>Data: ufcstats.com via Kaggle 2015\u20132025<br />XGBoost + Optuna + SHAP TreeExplainer</div>
        </div>
      </motion.div>
    </div>
  );
...
function FighterInput({ label, color, value, onChange, onSelect }) {
  ...
  return (
    <div ref={ref} style={{ flex: 1, position: "relative" }}>
      <div style={{ fontSize: 9, color: "#252525", letterSpacing: "0.18em", marginBottom: 7, display: "flex", alignItems: "center", gap: 8 }}>
        {label}
        <span style={{ padding: "1px 6px", borderRadius: 20, fontSize: 8, fontWeight: 700, background: color === "blue" ? "rgba(55,138,221,0.1)" : "rgba(216,90,48,0.1)", color: accent }}>{color.toUpperCase()}</span>
      </div>
      <input value={value} onChange={e => search(e.target.value)} onFocus={() => { setFocused(true); if (value.length >= 2) setOpen(results.length > 0); }} onBlur={() => setFocused(false)} onKeyDown={e => { if (e.key === "Enter" && results.length) pick(results[0]); }} placeholder="search fighter..." autoComplete="off"
        style={{ width: "100%", padding: "13px 16px", background: "#0a0a0a", border: `1px solid ${focused ? accent : "#181818"}`, borderRadius: 8, color: "#f0f0ec", fontSize: 14, fontFamily: "'DM Mono', monospace", outline: "none", transition: "border-color 0.2s" }} />
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.14 }}
            style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, right: 0, background: "#0f0f0f", border: "1px solid #181818", borderRadius: 8, zIndex: 200, overflow: "hidden", maxHeight: 210, overflowY: "auto" }}>
            {results.map(name => (
    ...
    <div style={{ minHeight: "100vh", background: "#080808", fontFamily: "'DM Mono', monospace", display: "flex", flexDirection: "column" }}>
      <nav style={{ padding: "13px 32px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid #0f0f0f", background: "rgba(8,8,8,0.96)", position: "sticky", top: 0, zIndex: 50, backdropFilter: "blur(12px)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{ fontSize: 11, color: "#1e1e1e", letterSpacing: "0.12em" }}>FIGHT PREDICTOR</span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
          <span style={{ fontSize: 10, color: "#161616" }}>{user?.email}</span>
          <button onClick={() => supabase.auth.signOut()}
            style={{ background: "none", border: "1px solid #161616", borderRadius: 5, color: "#252525", fontSize: 10, padding: "4px 10px", cursor: "pointer", fontFamily: "'DM Mono', monospace", letterSpacing: "0.08em", transition: "all 0.15s" }}
            onMouseEnter={e => { e.currentTarget.style.borderColor = "#D85A30"; e.currentTarget.style.color = "#D85A30"; }}
            onMouseLeave={e => { e.currentTarget.style.borderColor = "#161616"; e.currentTarget.style.color = "#252525"; }}
          >SIGN OUT</button>
        </div>
      </nav>
      <div style={{ flex: 1, maxWidth: 880, margin: "0 auto", width: "100%", padding: "40px 24px 64px" }}>
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease }}>
          <div style={{ display: "flex", gap: 12, alignItems: "flex-end", marginBottom: 12 }}>
            ...
            <div style={{ paddingBottom: 13, color: "#181818", fontSize: 11, fontWeight: 700, flexShrink: 0 }}>VS</div>
          ...
          <motion.button onClick={predict} disabled={loading} whileHover={{ opacity: loading ? 1 : 0.88 }} whileTap={{ scale: loading ? 1 : 0.98 }}
            style={{ width: "100%", padding: "14px 0", marginBottom: 36, background: loading ? "#0f0f0f" : "#e8ff5a", color: loading ? "#222" : "#000", border: "none", borderRadius: 8, fontSize: 11, fontWeight: 700, fontFamily: "'DM Mono', monospace", cursor: loading ? "not-allowed" : "pointer", letterSpacing: "0.14em", transition: "background 0.2s, color 0.2s" }}
          >{loading ? "ANALYZING..." : "PREDICT WINNER \u2192"}</motion.button>
        </motion.div>

        {!result && !loading && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
            style={{ textAlign: "center", padding: "80px 0", fontSize: 10, color: "#111", letterSpacing: "0.2em" }}>TYPE TWO NAMES \u00b7 HIT PREDICT</motion.div>
        )}
          ...
          {result && (
            <motion.div key="r" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.5, ease }} style={{ display: "flex", flexDirection: "column", gap: 1 }}>
              <div style={{ background: "#0b0b0b", border: "1px solid #161616", borderRadius: 10, padding: "24px 24px 20px" }}>
                <div style={{ fontSize: 9, color: "#252525", letterSpacing: "0.2em", marginBottom: 10 }}>PREDICTED WINNER</div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 1, marginTop: 1 }}>
                {result.shap_breakdown?.length > 0 && (
                  <div style={{ background: "#0b0b0b", border: "1px solid #161616", borderRadius: 10, padding: 22 }}>
                    <div style={{ fontSize: 9, color: "#252525", letterSpacing: "0.2em", marginBottom: 16 }}>SHAP BREAKDOWN</div>
                      ...
                      return (
                        <motion.div key={item.feature} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.06, duration: 0.4 }} style={{ marginBottom: 10 }}>
                          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, marginBottom: 3 }}>
                            ...
                            <span style={{ color: "#1a1a1a", fontSize: 10 }}>{item.contrib > 0 ? "+" : ""}{item.contrib.toFixed(3)} \u00b7 {name}</span>
                          </div>
                    ...
                    <div style={{ display: "flex", gap: 16, marginTop: 14, fontSize: 9, color: "#1a1a1a" }}>
                ...
                {result.stats?.length > 0 && (
                  <div style={{ background: "#0b0b0b", border: "1px solid #161616", borderRadius: 10, padding: 22 }}>
                    <div style={{ fontSize: 9, color: "#252525", letterSpacing: "0.2em", marginBottom: 16 }}>STAT COMPARISON</div>
              ...
              <div style={{ textAlign: "center", paddingTop: 20, fontSize: 9, color: "#111" }}>64.2% CV \u00b7 10,006 fights \u00b7 ufcstats.com via Kaggle 2015\u20132025</div>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
